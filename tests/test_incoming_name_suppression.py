...
class IncomingNameSuppressionTests(unittest.TestCase):
    ...
    def test_merged_incoming_events_keep_name_hiding(self):
        ...
        merged_name_setting = "hideNames or combatEvent.hideNames"

        # Then
        self.assertIn(stored_name_setting, source)
        self.assertIn(merged_name_setting, source)

    def test_unit_combat_healing_does_not_append_guessed_source_name(self):
        # Given
        source = MAIN_FILE.read_text(encoding="utf-8")
        queue_start = source.index("local function QueueIncomingHealBatch")
        queue_end = source.index("function eventFrame:UNIT_COMBAT", queue_start)

        # When
        incoming_heal_batch = source[queue_start:queue_end]
